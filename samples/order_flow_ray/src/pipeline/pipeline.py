    ...
    def run(self, files_slice=slice(None), specific_files: list[str] | None = None, specific_date_types: list[tuple[str, str]] | None = None):
        ...
        try:
            ...
            if self.config.processing.feature_engineering:
                ...
                metadata_path = self.config.storage.metadata.get_path()
                try:
                    ...
                    self.data_access.write_inventory('feature_engineering', discovered_files, metadata_path)
                except NoCredentialsError:
                    print("Discovery failed due to credentials, reading from inventory...")
                    discovered_files = self.data_access.read_inventory('feature_engineering', metadata_path)
